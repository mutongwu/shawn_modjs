...
menuTree.prototype={
	...
	editable:function(id){
		...
		function addItem(){//新增同级菜单
			...
			if( $nav.parent().is('.itembox')){
				var parentId = $nav.parent().find('a').attr('parentId');
			} else{
				var parentId = 0;
			}
			var appId = $(this).attr('appId');
			$copy.attr('id',$nav.attr('id')+'_sib'+iNUM++).find('a').attr('menuId',0).attr('value',0).html('新增菜单');
			$copy.find('span').removeClass();
			$(this).parent().parent().append($copy);
			_this.checkParent($nav,oId);
			bindRightClickMenu();
			//alert(parentId);
			editItem(true,appId,parentId);
		};
		function addSubItem(){//新增子级菜单
			...
			var parentId = $(this).attr('menuId');
			var appId = $(this).attr('appId');
			$copy.attr('id',$nav.attr('id')+'_sub'+iNUM++).find('a').attr('parentId',parentId).attr('menuId',0).attr('value',0).html('新增菜单');
			$copy.find('span').removeClass();
		...
		function editItem(flag,aid,pid){//编辑菜单
			...
			$.sysop.popup.confirm('编辑菜单',html,function(){
																																															   ...
																																															   if( flag == true){//新建节点
																																															   	var param = 'menuId=0&parentId='+pid+'&appId='+aid+'&name='+name+'&url='+url+'&hide='+hide;
																																															   	//alert(param);
																																															   	 fn_addItem(param);
																																															   	}
																																															   	  else{ //编辑节点
			...
			var mouseupHandler=function(){
				...
				if($tarTop.length){
					...
					if( $tarTop.parent('.itembox').length){ //如果释放的位置是在itembox里，就子节点里
						//alert($tarTop.parent().prev().find('a').attr('menuId')+'top+++上');
						if( $sorce.is('.parent')){
							alert('最多两层节点！');
						...
						$tarTop.before($sorce.find('a').attr('parentId',$tarTop.parent().prev().find('a').attr('menuId')).end());
					} else{//释放在根上
						//alert($tarTop.find('a').attr('menuId')+'top---下');
						if( $sorce.is('.parent')){ //如果拖拽的元素带有子节点 子节点pid不变
							$tarTop.before($sorce.find('.parent a').attr('parentId','0').end());
						} else{
							$tarTop.before($sorce.find('a').attr('parentId','0').end());
						}
				...
				if($tarMid.length){
					if( $sorce.is('.parent') || $tarMid.parent('.itembox').length){ //如果拖拽的元素带有子节点 子节点pid不变
							...
							return;
							//$tarBtm.after($sorce.find('.parent a').attr('parentId',$tarBtm.parent().prev().find('a').attr('menuId')).end());
						}
					if($tarMid.next('.itembox').length) { //释放的地方是父节点，即含有子节点
						//if( $tarMid.parent('.itembox').length>=1){ alert('最多两层！'); return;}
						//alert($tarMid.find('a').attr('menuId'));
						$tarMid.next('.itembox').append($sorce.find('a').attr('parentId',$tarMid.find('a').attr('menuId')).end());
					}
					else { //没包含子节点 生成一个 div.itembox
						//alert($tarMid.find('a').attr('menuId'));
						$tarMid.addClass('parent').after($('<div class="itembox"></div>').append($sorce.find('a').attr('parentId',$tarMid.find('a').attr('menuId')).end())).click();
					}
					//保存
					fn_editItem();
				};
				...
				if($tarBtm.length) { 
					if( $tarBtm.parent('.itembox').length){ //如果释放的位置是在itembox里，就子节点里
						//alert($tarBtm.parent().prev().find('a').attr('menuId')+'btm+++上');
						if( $sorce.is('.parent')){ //如果拖拽的元素带有子节点 子节点pid不变
							alert('最多两层节点！');
							...
							//alert('拖拽的是单节点');
							$tarBtm.after($sorce.find('a').attr('parentId',$tarBtm.parent().prev().find('a').attr('menuId')).end());
						}
					} //else if( $tarBtm.is(':last')){
						//alert(1);
					//} 
					else{ //放到根节点下
						//alert($tarBtm.find('a').attr('menuId')+'btm---下');
						if( $sorce.is('.parent')){ //如果拖拽的元素带有子节点 子节点pid不变
							$tarBtm.after($sorce.find('.parent a').attr('parentId','0').end());
						} else{
							$tarBtm.after($sorce.find('a').attr('parentId','0').end());
						}
					}
	...
	parseItem:function(box){//解析数据(返回后台)
		...
		$o.children('.item-nav').each(function(){
			...
			//($treeNode.attr('appId') !== undefined) && (json.appId = $treeNode.attr('appId'));
			json.appId=$treeNode.attr('appId');
			json.parentId=$treeNode.attr('parentId');
			if( chlidTree){
				json.resourcesId=$treeNode.attr('resourcesId');
			} else{
				json.menuId=$treeNode.attr('menuId');
			}
